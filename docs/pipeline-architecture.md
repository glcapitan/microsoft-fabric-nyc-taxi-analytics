...
3. **Copy to staging.** The source file path is built dynamically — `concat('yellow_tripdata_', variables('v_date'), '.parquet')` — reading from the Lakehouse `nyctaxi_yellow` files folder into `stg.nyctaxi_yellow`. A pre-copy script deletes the staging table's contents first (`DELETE FROM`, as Fabric Warehouse does not yet support `TRUNCATE`), so staging always holds exactly one month.
...
5. **Outlier-date cleansing (Stored procedure).** The raw files contain stray records outside the target month (dates as far back as 2002 were observed in the January file). The parameterized `stg.data_cleaning_stg` procedure deletes rows where `tpep_pickup_datetime` falls outside the `v_date`–`v_end_date` window.
